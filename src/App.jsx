...
const performMatching = (regData, jumioData) => {
  ...
  report.checks.push({ field: 'Date of Birth', reg: regData.dob, doc: docData.dob, match: dobMatch, message: dobMatch ? 'Match' : 'Mismatch' });
  const regPC = normalizePostcode(regData.postcode);
  const docPC = normalizePostcode(docData.postcode);
  const pcMatch = regPC === docPC;
  report.checks.push({ field: 'Postcode', reg: regPC, doc: docPC, match: pcMatch, message: pcMatch ? 'Match' : 'Mismatch' });
  const lnSim = calculateSimilarity(regData.lastName, docData.lastName);
  const lnMatch = lnSim >= 85;
  report.checks.push({ field: 'Last Name', reg: regData.lastName, doc: docData.lastName, match: lnMatch, message: `Similarity: ${lnSim.toFixed(0)}%` });
  const fnSim = calculateSimilarity(regData.firstName, docData.firstName);
  const fnMatch = fnSim >= 85;
  report.checks.push({ field: 'First Name', reg: regData.firstName, doc: docData.firstName, match: fnMatch, message: `Similarity: ${fnSim.toFixed(0)}%` });
  const adSim = calculateSimilarity(regFullAddress, docData.addressLine);
  const adMatch = adSim >= 70;
  report.checks.push({ field: 'Address Match', reg: regFullAddress, doc: docData.addressLine, match: adMatch, message: `Similarity: ${adSim.toFixed(0)}%` });
  const citySim = calculateSimilarity(regData.city, docData.city);
  const cityMatch = citySim >= 80;
  report.checks.push({ field: 'City', reg: regData.city, doc: docData.city, match: cityMatch, message: `Similarity: ${citySim.toFixed(0)}%` });

  let docValid = true;
  const isUKDL = docData.type === 'DRIVING_LICENSE' && docData.country === 'GBR';
  if (!isUKDL) {
    report.checks.push({ field: 'Document Rules', reg: 'UK Logic', doc: `${docData.country} ${docData.type}`, match: false, message: 'Non-UK Driving License. Proof of Address required.' });
    docValid = false; 
  } else {
    report.checks.push({ field: 'Document Rules', reg: 'UK Logic', doc: 'UK Driving License', match: true, message: 'Address verified via License.' });
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-800 font-sans selection:bg-green-100">
      ...
      <main className="max-w-xl mx-auto p-6 mt-8">
          ...
          <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
            ...
            <div className="space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1"><label className="text-xs font-semibold text-gray-500 uppercase">First Name</label><input type="text" className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 outline-none" value={regData.firstName} onChange={e => setRegData({...regData, firstName: e.target.value})} /></div>
                <div className="space-y-1"><label className="text-xs font-semibold text-gray-500 uppercase">Last Name</label><input type="text" className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 outline-none" value={regData.lastName} onChange={e => setRegData({...regData, lastName: e.target.value})} /></div>
              </div>
              <div className="space-y-1"><label className="text-xs font-semibold text-gray-500 uppercase">Date of Birth</label><div className="relative"><input type="date" className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 outline-none" value={regData.dob} onChange={e => setRegData({...regData, dob: e.target.value})} /><Calendar className="absolute right-3 top-3 text-gray-400 pointer-events-none" size={18} /></div></div>
              <div className="border-t border-gray-100 my-4"></div>
              <div className="flex gap-4">
                <div className="w-1/4 space-y-1"><label className="text-xs font-semibold text-gray-500 uppercase">No</label><input type="text" className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 outline-none" placeholder="10" value={regData.houseNo} onChange={e => setRegData({...regData, houseNo: e.target.value})} /></div>
                <div className="w-3/4 space-y-1"><label className="text-xs font-semibold text-gray-500 uppercase">Street</label><input type="text" className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 outline-none" placeholder="Downing Street" value={regData.street} onChange={e => setRegData({...regData, street: e.target.value})} /></div>
              </div>
              <div className="space-y-1"><label className="text-xs font-semibold text-gray-500 uppercase">Flat number</label><input type="text" className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 outline-none" placeholder="Apt 4B (Optional)" value={regData.flat} onChange={e => setRegData({...regData, flat: e.target.value})} /></div>
              <div className="flex gap-4">
                <div className="w-1/3 space-y-1"><label className="text-xs font-semibold text-gray-500 uppercase">Postcode</label><input type="text" className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 outline-none" placeholder="SW1A 2AA" value={regData.postcode} onChange={e => setRegData({...regData, postcode: e.target.value})} /></div>
                <div className="w-2/3 space-y-1"><label className="text-xs font-semibold text-gray-500 uppercase">City</label><input type="text" className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 outline-none" placeholder="London" value={regData.city} onChange={e => setRegData({...regData, city: e.target.value})} /></div>
              </div>
            </div>
            <button onClick={() => setStep(2)} disabled={!regData.postcode || !regData.street || !regData.houseNo} className="mt-8 w-full bg-green-600 text-white py-4 rounded-xl font-bold hover:bg-green-700 shadow-lg shadow-green-100 transition disabled:opacity-50 disabled:cursor-not-allowed">Next Step</button>
          </div>
        )}
        {step === 2 && (
          <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 text-center py-12">
            <div className="w-16 h-16 bg-amber-50 rounded-full flex items-center justify-center mx-auto mb-6"><ShieldAlert size={32} className="text-amber-500" /></div>
            ...
            <button onClick={() => { setStep(3); startJumio(); }} className="w-full bg-green-600 text-white py-4 rounded-xl font-bold hover:bg-green-700 shadow-lg shadow-green-100 transition">Start ID Verification</button>
          </div>
        )}
        {step === 3 && (
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-200">
            <div className="bg-slate-900 text-white p-4 font-bold flex items-center gap-2"><ShieldCheck size={18}/> Identity Verification</div>
        ...
        {step === 4 && matchReport && (
          <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
            <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-6 ${matchReport.passed ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>{matchReport.passed ? <Check size={32} /> : <X size={32} />}</div>
